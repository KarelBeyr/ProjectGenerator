...
public class ControllersGenerator : GeneratorBase
{
    ...
    public void Generate(DataModel dm)
    {
        foreach (var cls in dm.Classes.Values.Where(e => e.IsModel))
        {
            ...
            var autogeneratedPrimaryKey = cls.PrimaryKeyFields().SingleOrDefault(e => e.IsAutogeneratedKey);
            var notAutogeneratedPrimaryKeys = cls.PrimaryKeyFields().Where(e => !e.IsAutogeneratedKey && e.ControllerFromHeader == null);
            ...
            sb.IncreaseIndent();
            foreach (var field in cls.Fields.Where(e => !(e.IsPrimaryKey && e.IsAutogeneratedKey) && !e.IsOnlyInDb && e.ControllerFromHeader == null))
            {
            ...
            parameters = string.Join('/', cls.PrimaryKeyFields().Where(e => !e.IsOptional && e.ControllerFromHeader == null).Select(e => $"{{{e.Name}}}"));
            sb.AppendLine($"[HttpGet(\"{parameters}\")]");
